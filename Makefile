...
# Quest constants and tables are generated by a Python script
QUESTS_TOOL_DIR := $(TOOLS_DIR)/quests
QUESTS_DATA_DIR := $(DATA_SRC_SUBDIR)/quests
NOTES_DATA_DIR := $(DATA_SRC_SUBDIR)/notes
AUTO_GEN_TARGETS += $(INCLUDE_DIRS)/constants/quests.h $(DATA_SRC_SUBDIR)/quests.h

$(INCLUDE_DIRS)/constants/quests.h $(DATA_SRC_SUBDIR)/quests.h &: $(wildcard $(QUESTS_DATA_DIR)/*.json) $(wildcard $(NOTES_DATA_DIR)/*.json) $(QUESTS_TOOL_DIR)/quests_to_header.py $(INCLUDE_DIRS)/constants/quest_types.h
	python3 $(QUESTS_TOOL_DIR)/quests_to_header.py
